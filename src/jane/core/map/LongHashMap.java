...
public class LongHashMap<V> implements Cloneable
{
	...
	{
		if (key == EMPTY)
			return _hasZeroValue ? _zeroValue : defaultValue;
		int index = (int)key & _mask;
		if (_keyTable[index] != key)
		{
			index = hash2(key);
	...
	{
		if (maximumCapacity > _capacity)
			return;
		if (maximumCapacity < _size)
			maximumCapacity = _size;
	...
	public boolean containsValue(Object value, boolean identity)
	{
		...
		if (value == null)
		{
			if (_hasZeroValue && _zeroValue == null)
				return true;
			long[] kt = _keyTable;
			for (int i = 0, n = _tableSize; i < n; ++i)
		...
		{
			if (_hasZeroValue && value.equals(_zeroValue))
				return true;
			for (int i = 0, n = _tableSize; i < n; ++i)
				if (value.equals(vt[i]))
					return true;
		}
		return false;
	}
	...
	public void ensureCapacity(int additionalCapacity)
	{
		...
		if (sizeNeeded >= _threshold)
			resize(IntHashMap.nextPowerOfTwo((int)(sizeNeeded / _loadFactor)));
	}
