 ...
 *
 * <p>A {@link Set} projection of a LongConcurrentHashMap may be created
 * (using {@link #newKeySet()} or {@link #newKeySet(int)}), or viewed
 * (using {@link #keySet(Object)} when only keys are of interest, and the
 * mapped values are (perhaps transiently) not used or all take the
 * same mapping value.
 *
 * <p>A LongConcurrentHashMap can be used as scalable frequency map (a
 * form of histogram or multiset) by using {@link
 * java.util.concurrent.atomic.LongAdder} values and initializing via
 * {@link #computeIfAbsent computeIfAbsent}. For example, to add a count
 * to a {@code LongConcurrentHashMap<String,LongAdder> freqs}, you can use
 ...
 * from those of LongConcurrentHashMap: Any non-null result returned
 ...
 */
public final class LongConcurrentHashMap<V> {
    ...
    static class Node<V> {
        ...
        public final int hashCode()   { return (int)key ^ val.hashCode(); }
        ...
        public final boolean equals(Object o) {
            long k; Object v, u; Node<?> e;
            return ((o instanceof Node) &&
                    (v = (e = (Node<?>)o).getValue()) != null &&
                    (e.getKey() == key) &&
                    (v == (u = val) || v.equals(u)));
        }
     ...
     * @return the set view
     */
    public EntrySetView<V> entrySet() {
        EntrySetView<V> es;
    ...
    static final class TreeNode<V> extends Node<V> {
        ...
        final TreeNode<V> findTreeNode(int h, long k, Class<?> kc) {
            TreeNode<V> p = this;
            do  {
                int ph, dir; long pk; TreeNode<V> q;
    ...
    static class BaseIterator<V> extends Traverser<V> {
        final LongConcurrentHashMap<V> map;
        Node<V> lastReturned;
        BaseIterator(Node<V>[] tab, int size, int index, int limit,
                    LongConcurrentHashMap<V> map) {
    ...
    static final class KeyIterator<V> extends BaseIterator<V> {
        ...
        public final long nextElement() { return next(); }
    }
    ...
    static final class EntryIterator<V> extends BaseIterator<V>
        implements Iterator<Map.Entry<V>> {
        EntryIterator(Node<V>[] tab, int index, int size, int limit,
                      LongConcurrentHashMap<V> map) {
            super(tab, index, size, limit, map);
        }

        public final Map.Entry<V> next() {
            Node<V> p;
            if ((p = next) == null)
                throw new NoSuchElementException();
            long k = p.key;
     ...
     */
    static final class MapEntry<V> implements Map.Entry<V> {
        final long key; // non-null
        V val;          // non-null
        final LongConcurrentHashMap<V> map;
        MapEntry(long key, V val, LongConcurrentHashMap<V> map) {
            this.key = key;
            this.val = val;
            this.map = map;
        }
    ...
    abstract static class CollectionView<V,E>
        ...
        final LongConcurrentHashMap<V> map;
        CollectionView(LongConcurrentHashMap<V> map)  { this.map = map; }
         ...
         */
        public LongConcurrentHashMap<V> getMap() { return map; }
    ...
    /**
     * A view of a LongConcurrentHashMap as a {@link Set} of keys, in
     ...
     * @since 1.8
     */
    public static class KeySetView<V> extends CollectionView<V,K> {
        private final V value;
        KeySetView(LongConcurrentHashMap<V> map, V value) {  // non-public
            super(map);
            this.value = value;
        }
    ...
    /**
     * A view of a LongConcurrentHashMap as a {@link Collection} of
     * values, in which additions are disabled. This class cannot be
     * directly instantiated. See {@link #values()}.
     */
    static final class ValuesView<V> extends CollectionView<V,V>
        implements Collection<V> {
        ValuesView(LongConcurrentHashMap<V> map) { super(map); }
    ...
    /**
     * A view of a LongConcurrentHashMap as a {@link Set} of (key, value)
     * entries.  This class cannot be directly instantiated. See
     * {@link #entrySet()}.
     */
    public static final class EntrySetView<V> extends CollectionView<K,V,Map.Entry<V>> {
        EntrySetView(LongConcurrentHashMap<V> map) { super(map); }
         ...
         */
        public Iterator<Map.Entry<V>> iterator() {
            LongConcurrentHashMap<V> m = map;
            Node<V>[] t;
            int f = (t = m.table) == null ? 0 : t.length;
