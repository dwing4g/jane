...
rpc { name="TestRPCBean2", type=5, arg="TestBean", res="TestBean",
}

dbt { name="TestTable", lock="test", key="id", value="TestType", cachesize=200000, comment="数据库表定义. key类型只能是32/64位整数/浮点数或字符串/binary类型或bean类型, id类型表示优化的非负数long类型" }
dbt { name="BeanTable", lock="bean", key="TestBean", value="TestBean", cachesize=65536, comment="value类型必须是bean定义的类型" }
dbt { name="OctetsTable", lock="bean", key="binary", value="TestEmpty", cachesize=1000, memory=true, comment="注意表名和key类型的对应关系是不能改变的" }
